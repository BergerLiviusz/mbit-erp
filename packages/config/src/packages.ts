...
export interface PackageModuleFlags {
  documents: boolean;
  team: boolean;
  crm: boolean;
  logistics: boolean;
...
export interface ErpPackageDefinition {
  /** Belső azonosító (build env, artifact név) */
  id: string;
  /** Marketing / UI megjelenítés */
  displayName: string;
  editionLabel: string;
  version: string;
  modules: PackageModuleFlags;
  buildName: string;
  artifactSlug: string;
  /** Régi package-* aliasok (CI / branch kompatibilitás) */
  legacyIds?: string[];
}

/** NestJS / API route prefix → modul */
export const API_ROUTE_MODULE_MAP: Record<string, ErpModuleKey> = {
  crm: 'crm',
  dms: 'documents',
  logistics: 'logistics',
  hr: 'hr',
...
export const ERP_PACKAGES: Record<string, ErpPackageDefinition> = {
  full: {
    id: 'full',
  ...
  'package-4': {
    id: 'package-4',
    displayName: 'Csomag 4',
    editionLabel: 'Csomag 4',
    version: APP_VERSION,
    modules: {
      documents: true,
      team: true,
...
export function getPackageDefinition(packageId?: string | null): ErpPackageDefinition {
...
export function getActivePackageIdFromEnv(): string {
  return resolvePackageId(
    process.env.ERP_PACKAGE || process.env.VITE_ACTIVE_PACKAGE || 'full',
  );
}

/** Permission prefix → modul (package szűréshez) */
export const PERMISSION_MODULE_PREFIX: Record<string, ErpModuleKey> = {
  crm: 'crm',
  customer: 'crm',
  opportunity: 'crm',
  quote: 'crm',
