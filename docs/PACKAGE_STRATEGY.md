...
npm run package:hr-only            # v1.0.1g – csak HR
```
...
| `hr-only` | `mbit-erp-v1.0.1g-{sha}-hr-only-HR-windows.zip` |
...
| `release/hr-only` | `hr-only` |
| `package-hr` | `hr` (legacy) |
| `main` / `master` | `full` |
